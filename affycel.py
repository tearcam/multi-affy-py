...
class affycel:
    ...
    if __name__ == "__main__":        
        ...
        testlist = (self.filename, self.version, self.header.items(), self.intensityCells, self.intensity[:5], self.masksCells, self.masks, self.outliersCells, self.outliers[:5], self.modifiedCells, self.modified[:5])
            ...
            for row in reader:
                if row:
                    if not any(("[CEL]" in row, "[HEADER]" in row, "[INTESITY]" in row, "[MASKS]" in row, "[OUTLIERS]" in row)):
                        ...
                        self.modified[masks] = numpy.array(data, [('x', int), ('y', int), ('origmean', numpy.float64)] )
